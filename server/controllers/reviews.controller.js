...
const { mapReview } = require('../utils/helpers');

async function isEligibleToReview(userId, roomId) {
...
async function createOrUpdateReview(req, res) {
  ...
  if (!ratingNum || ratingNum < 1 || ratingNum > 5) return res.status(400).json({ error: 'Rating must be between 1 and 5' });
  ...
  try {
    ...
    if (!eligible) return res.status(403).json({ error: 'Only tenants who have rented this room can leave a review' });
...
async function getRoomReviews(req, res) {
  try {
    ...
    res.json({ items: reviews, average: Math.round(average * 10) / 10, count });
  } catch (e) {
...
async function myReview(req, res) {
  try {
    ...
    res.json(result.rows[0] ? mapReview(result.rows[0]) : null);
  } catch (e) {
...
async function deleteReview(req, res) {
  try {
    const result = await pool.query('DELETE FROM reviews WHERE id=$1 AND tenant_id=$2', [req.params.id, req.user.userId]);
    if (result.rowCount === 0) return res.status(404).json({ error: 'Review not found or not yours' });

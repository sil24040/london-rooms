...
function updateNav() {
  ...
  document.getElementById('nav-user').textContent = li ? 'Hi, ' + user.name.split(' ')[0] : '';
}
...
async function loadRooms() {
  ...
  try {
    ...
    document.getElementById('result-count').textContent = res.total + (res.total === 1 ? ' room' : ' rooms');
...
function renderCard(r) {
  ...
  return `<div class="card">
    ...
    <div class="meta">${escapeHtml(r.type)} · ${r.billsIncluded ? 'Bills incl.' : 'Bills not incl.'} ${r.availableNow ? '· <span class="badge badge-avail">Available now</span>' : ''}</div>
...
async function showDetail(id) {
  ...
  try {
    ...
    document.getElementById('detail-content').innerHTML = `
      ...
      <p class="meta" style="margin-bottom:1rem">${escapeHtml(r.type)} · ${r.billsIncluded?'Bills included':'Bills not included'} · ${r.availableNow?'Available now':'Coming soon'}</p>
...
async function loadDashboard() {
  if (!user) { showPage('login'); return; }
  document.getElementById('dash-title').textContent = 'Welcome back, ' + user.name;
  document.getElementById('dash-sub').textContent = user.role === 'landlord' ? 'Landlord dashboard' : 'Tenant dashboard';
...
async function loadTenantEnquiries() {
  ...
  try {
    ...
    el.innerHTML = !list.length ? '<div class="empty"><span class="icon">✉️</span><p>No enquiries sent yet</p></div>' : list.map(e=>`
      <div class="card" style="margin-bottom:8px">
        ...
        ${e.reply ? `<div style="margin-top:8px;padding:8px;background:#f5f5f3;border-radius:8px;font-size:13px"><strong>Landlord reply:</strong><br>${escapeHtml(e.reply)}</div>` : ''}
      </div>`).join('');
  } catch(e) { el.innerHTML = '<div class="alert alert-error">'+e.message+'</div>'; }
}
